...
const verifyCode = catchError(async(req,res)=>{
  ...
  if(!codeFound) return res.status(401).json({data: "Invalid Code"})
...
const login = catchError(async(req,res)=>{
  ...
  if(!user) return res.status(401).json({data: "Invalid Credentials"})
  const isValid = await bcrypt.compare(password, user.password);
  if(!isValid) return res.status(401).json({data: "Invalid Credentials"})
  if(!user.isVerified) return res.status(401).json({data: "Invalid Credentials"})
